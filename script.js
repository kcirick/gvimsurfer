...
function SurferHint(element, offset) {
...
SurferNumberHint.prototype.getTextHint = function (i, length) {
   start = length <=10 ? 1 : length <= 100 ? 10 : 100;
   var content = document.createTextNode(start + i);
   this.hint.appendChild(content);
}

SurferNumberHint.prototype.betterMatch = function(input) {
   var bestposition = 37;
   var ret = 0;
...
SurferNumberHint.prototype.matchText = function(input) {
   var ret = false;
...
SurferLetterHint.prototype.getTextHint = function(i, length) {
   var text;
...
SurferLetterHint.prototype.betterMatch = function(input) {
   return 0;
}

SurferLetterHint.prototype.matchText = function(input) {
   return (this.hint.textContent.toLowerCase().indexOf(input.toLowerCase()) == 0);
}

function SurferLetterHint(element, offset) {
   this.constructor = SurferHint;
   this.constructor(element, offset);
}

function SurferNumberHint(element, offset) {
   this.constructor = SurferHint;
   this.constructor(element, offset);
}
...
function create_stylesheet() {
   ...
   style.insertRule('*[surfer_highlight=hint_normal] { background: ' + hint_normal_color + ' !important; } ', 0);
   style.insertRule('*[surfer_highlight=hint_active] { background: ' + hint_active_color + ' !important } ', 0);
}
...
function show_hints() {
   ...
   hints.id = "surfer_hints";
   var constructor = hint_style.toLowerCase() == "letter" ? SurferLetterHint : SurferNumberHint;
   ...
   for (var i=0; i<elements.length; i++) {
      ...
      e.element.setAttribute('surfer_highlight', 'hint_normal');
   }
   active_arr = elements;
...
function update_hints(input) {
   ...
   for (var i=0; i<active_arr.length; i++) {
      ...
      if (e.matchText(input)) {
         ...
         e.element.removeAttribute('surfer_highlight');
      }
   }
   active_arr = array;
   active = array[0];
...
function set_active(element) {
   var active = document.querySelector('*[surfer_highlight=hint_active]');
   if (active) {
      active.setAttribute('surfer_highlight', 'hint_normal' );
   }
   element.element.setAttribute('surfer_highlight', 'hint_active');
   active = element;
}
...
function clear() {
   if (elements) {
      for (var i=0; i<elements.length; i++) {
         elements[i].element.removeAttribute('surfer_highlight');
      }
   }
   var hints = document.getElementById("surfer_hints");
   if (hints) 
...
function focus_input(){
   if(document.getElementsByTagName("body")[0]===null||
         typeof(document.getElementsByTagName("body")[0])!="object"){
            return;
         }

   var r=document.evaluate("//input[@type='text']|//input[@type='password']|//textarea", 
         document, 
   ...
   for(i=0; i<r.snapshotLength; i++){
      ...
      if(elem==document.activeElement){ j=1; }
      k++;
   }
   if(j===0&&first!==null){
      first.focus();
